...
function getMapping(schema, mapOrGenerator) {
  mapOrGenerator = mapOrGenerator || generator.generateMapping;
  if (typeof mapOrGenerator == "function") {
    var retMapping = {};
    mapOrGenerator(schema, (err, mapping) => {
      retMapping = mapping;
    });
    return retMapping;
  } else 
      throw new TypeError("Mapping function is wrong type");
...
function createMappingIfNotPresent(options, cb) {
  var client = options.client,
    ...
    mapGen = (typeof options.mapping == "function")? options.mapping : generator.generateMapping;

    mapGen(schema, (ignoredErr, mapping) => {
    var completeMapping = {};
...
function Mongoosastic(schema, pluginOpts) {
  ...
  var bulkTimeout, bulkBuffer = [], esClient,
    ...
    mapFunction = pluginOpts.mapping || generator.generateMapping,
    mapping = getMapping(schema, mapFunction),
